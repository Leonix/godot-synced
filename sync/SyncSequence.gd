...
func _physics_process(_delta):
	...
	if SyncManager.is_client():
		...
		_input_id_to_state_id.write(SyncManager.get_local_peer().input_id, get_interpolation_state_id())

	if SyncManager.is_server():
		_input_id_to_state_id.write(SyncManager.get_local_peer().input_id, state_id)
		last_consumed_input_id = SyncManager.get_local_peer().input_id
...
func calculate_time_depth(target_coord):
	...
	for wr in _synced_belong_to_players:
		...
		var coord = SyncManager.get_coord(synced.get_parent())
...
func _td_calc(players: Array):
	...
	var half_delay = state_id - SyncManager.get_peer(closest_peer_id).state_id
	return [
...
var _input_id_to_state_id = SyncPeer.CircularBuffer.new(
	int(max(
		SyncManager.client_interpolation_history_size, 
		SyncManager.input_frames_history_size
	)), 
	0
)
...
func get_interpolation_state_id()->int:
	assert(SyncManager.is_client())
	var result = state_id - SyncManager.client_interpolation_lag
