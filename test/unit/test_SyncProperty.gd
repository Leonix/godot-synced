...
func test_no_interpolation():
	...
	assert_eq([100,100,100,100,100,100,100,100,100,100], prop.container)
	prop.write(15, 200)
	assert_eq(15, prop.last_state_id)
	assert_eq(1, prop.container.count(200))
	assert_eq(200, prop.container[prop.last_index])
	assert_eq(200, prop.read(15)) # int state id, no interpolation
	assert_eq(200, prop.read(16)) # (no)extrapolation
	assert_eq(100, prop.read(14.5)) # (no)interpolation between different values
	assert_eq(100, prop.read(13.5)) # (no)interpolation between equal values
	assert_eq(100, prop.read(1)) # value older than stored
...
func test_linear_interpolation():
	...
	assert_eq(118, prop.container[prop.last_index])
	assert_eq(114, prop.read(14)) # int state id, no interpolation
	assert_eq(120, prop.read(20)) # extrapolation into future int state
	assert_eq(120.3, prop.read(20.3)) # extrapolation into future float state
	assert_eq(115, prop.read(15)) # interpolation between different values, int
	assert_eq(113.5, prop.read(13.5)) # interpolation between different values, float
	assert_eq(113, prop.read(13)) # oldest value stored
	assert_eq(113, prop.read(12)) # value older than stored
	
	prop.write(19, 118)
	assert_eq(19, prop.last_state_id)
	assert_eq(118, prop.read(18.5)) # interpolation between equal values
	#gut.p('%s; last_state_id=%s, last_index=%s' % [prop.container, prop.last_state_id, prop.last_index])
...
func test_get_index():
	...
	for i in range(12, 16):
		assert_eq(100+i, prop.container[prop._get_index(i)])
