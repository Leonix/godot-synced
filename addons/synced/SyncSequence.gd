...
var state_id_frac: get = get_state_id_frac

# Client: same as state_id but takes interpolation lag into account
var interpolation_state_id: get = get_interpolation_state_id

# Client: same as state_id_frac but takes interpolation lag into account
var interpolation_state_id_frac: get = get_interpolation_state_id_frac
...
func update_synced_belong_to_players(before, after, synced: Synced):
	...
	if before != null and after == null:
		...
		if not synced.synced_property('_td_position'):
			...
			if synced.get_parent() is Node2D or synced.get_parent() is Node3D:
				synced.add_synced_property('_td_position', SyncedProperty.new({
					missing_state_interpolation = SyncedProperty.Interpolation.NO_INTERPOLATION,
					interpolation = SyncedProperty.Interpolation.NO_INTERPOLATION,
					sync_strategy = SyncedProperty.Strategy.DO_NOT_SYNC,
					auto_sync_property = 'translation' if synced.get_parent() is Node3D else 'position'
				}))
...
func _td_calc(players: Array):
	...
	players.sort_custom(_td_calc_sorter)
...
func update_received_state_id_and_mtime(new_server_state_id, consumed_input_id):
	...
	_last_received_state_mtime = Time.get_ticks_msec()
